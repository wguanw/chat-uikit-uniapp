  ...
  <view class="manage" ref="dialog">
    ...
    <view class="main" v-if="!currentTab">
      ...
      <view class="userInfo space-top">
        <view class="userInfo-header" @click="setTab('member')">
          ...
          <view style="display: flex">
            <view
              >{{ conversation.groupProfile?.memberCount?.memberCount }}人</view
            >
            <image
              class="image-right"
        ...
        <view style="display: flex">
          ...
          <view
            style="padding: 10px"
            v-if="conversation.groupProfile.selfInfo.role === 'Owner'"
          >
      ...
      <view class="content space-top" @click="editLableName = ''">
        ...
        <view class="item-li">
          <view>群类型</view>
          <view>{{ typeName[conversation.groupProfile.type] }}</view>
        </view>
        <view class="item-li">
          <view>加群方式</view>
          <view>{{ typeName[conversation.groupProfile.joinOption] }}</view>
        </view>
      </view>
    ...
    <ManageMember
      ...
      :list="userInfo.list"
      :total="conversation.groupProfile.memberCount?.memberCount"
...
const TUIGroupManage = defineComponent({
  ...
  setup(props: any, ctx: any) {
    ...
    const getMember = (type?: string) => {
      ...
      uni.$TUIKit.getGroupMemberList(options).then((res: any) => {
        if (type && type === "more") {
    ...
    const quit = async (group: any) => {
      await uni.$TUIKit.quitGroup(group.groupID);
      uni.navigateTo({
        url: "../../TUIPages/TUIConversation/index",
      });
    };

    // 解散群
    const dismiss = async (group: any) => {
      await uni.$TUIKit.dismissGroup(group.groupID);
    ...
    const updateProfile = async (params: any) => {
      ...
      const res = await uni.$TUIKit.updateGroupProfile(options);
      const { conversation } = TUIGroupServer.store;
    ...
    const handleOperateMember = (type) => {
      if (type) {
        uni.navigateTo({
          url: `../TUIGroup/memberOperate?type=${type}&groupID=${data.conversation.groupProfile.groupID}`,
        });
      }
    };
