  ...
  <view class="TUI-Create-conversation-container">
    <view class="tui-search-area">
      <view class="tui-search-bar">
        ...
        <input
          ...
          @confirm="handleGetUserProfileInfo"
          @blur="handleGetUserProfileInfo"
        />
      </view>
    ...
    <view class="tui-person-list-container">
      <view
        ...
        v-for="(item, index) in chooseUserList"
        :key="index"
      ...
      uni.$TUIKit
        ...
        .then((imRes) => {
          ...
          if (imRes.data.length > 0) {
            ...
            if (
              data.chooseUserList.filter(
                (obj) => userInfo.userID === obj.userID
              ).length === 0
            ) {
          ...
          case uni.$TIM.TYPES.GRP_MEETING: {
            ...
            if (chooseList.length > 2) {
              name =
                chooseList
                  .slice(0, 3)
                  .map((obj) => obj.nick || obj.userID)
                  .join(",") || "";
            } else {
              name =
                chooseList.map((obj) => obj.nick || obj.userID).join(",") +
                "、" +
                (data.myProfile?.nick || data.myProfile?.userID);
            }
            const groupOptions = {
              avatar:
